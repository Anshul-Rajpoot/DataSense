...
problem_type = infer_problem_type(df[target_column])
st.caption(f"Detected problem type: {problem_type}")
st.caption("Choose a simple baseline model first, then compare more advanced models if needed.")
...
if row_count < 500:
    ...
    min_value = 500
    max_value = max(500, row_count)
    default_value = min(2000, row_count)

max_rows = st.number_input(
    "Max rows used for training comparison",
    min_value=min_value,
    max_value=max_value,
    value=default_value,
    step=250,
)
st.caption("Large datasets are sampled before comparison to keep the workflow responsive and easier to interpret.")
